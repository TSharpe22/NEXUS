...
the three tokens it uses. It was originally drawn by hand from DESIGN.md, which
still carries the pre-restyle palette, so the launcher icon sat a shade off the
app it opened until the drawing was committed as a script. **The icon is not
regenerated by `npm run dist`** — it is a committed asset, so run `npm run icon`
when the palette moves and commit what it writes.
