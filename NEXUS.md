...
The renderer holds that day as `today` in the store, and one module-scope
ticker in `store/app-store.ts` moves it: a 60s interval plus a `window` focus
listener, writing only when the date has actually changed. **It is the only
interval in the renderer and it is meant to stay the only one.** `useToday()`
used to compute the date inside a Zustand selector, and a selector re-runs
only when something writes to the store — so a window left open past the
day-start hour went on answering yesterday until an unrelated write happened
...
**`applySchema` is one transaction, all or nothing.** A file is either on the
version it started at or on the current one, never on something in between:
the v9 `links` rebuild drops a table and renames its replacement into place,
and run as bare statements SQLite committed each one as it went, so a crash in
that gap left a vault with no `links` table and a version the next launch read
as "nothing to rebuild". `PRAGMA user_version` is itself transactional, which
is what makes retrying safe. Two things must stay outside it: the backups,
because `backupDatabase` checkpoints the WAL and a checkpoint cannot run
inside a transaction, and `PRAGMA foreign_keys`, which is a no-op inside one
and is set by `database.ts` around the call. Every step that rewrites values
rather than adding tables takes a backup first — the legacy rebuild and the v6
relation repair are both such steps.
...
The icon itself is drawn by `npm run icon`, which writes `build/icon.png` from
the three tokens it uses — `--nx-bg`, `--nx-surface-sunken` and `--nx-accent`.
It was originally drawn by hand from DESIGN.md, and every colour change since
was a pixel remap of that one bitmap, so its neutrals kept tracing DESIGN.md's
pre-restyle greys and the launcher icon sat a shade off the app it opened.
**The icon is not regenerated by `npm run dist`** — it is a committed asset, so
run `npm run icon` when the palette moves and commit what it writes.

Nothing about a new palette reaches the desktop until the package is rebuilt
and reinstalled: `npm run dev` restyles the window you are looking at, but the
launcher icon comes from the installed `.deb`, not from the checkout.
...
- **`npm run check` before offering a branch.** Typecheck, the migration
  checker, the backup checker, and a smoke test that drives the real app
  through every view. `.github/workflows/check.yml` runs the same command on
  every push, so a branch that skipped it says so on GitHub rather than in the
  vault. The probes under `scripts/probes/` are not part of it — they measure
  rather than assert.
